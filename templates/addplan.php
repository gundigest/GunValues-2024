...
if($loggedIn){
	...
	if(($plan) && ($plan['status'] == "ongoing")){//there is an active plan and we should not be able to view the AddPlan form
		header("Location: " . $root . "account/?err=You currently have an active plan. You cannot have more than one plan at a time."); /* Redirect browser */
		exit();
	}
...
$html .= <<<EOD
{$error_banner}
		<h2 class="subhead">Choose Your Plan</h2>		
		<form action="{$root}addplan" METHOD="POST" id="update-form">			
			<input type="hidden" name="plan" id="plan"/>			
				...
				<div class="sub_plan" id="3-day">
					...
					<a href="javascript:void(0);" class="button center bright">Get 3-Day Access<span class="check">&#10004;</span></a>
				</div>			
				<div class="sub_plan" id="monthly">
					...
					<a href="javascript:void(0);" class="button center bright">Get Monthly Access<span class="check">&#10004;</span></a>
				</div>				
				<div class="sub_plan" id="yearly">
					...
					<a href="javascript:void(0);" class="button center bright">Get Annual Access<span class="check">&#10004;</span></a>
			...
			<div class="search-box-text">Billing Address</div>	
					...
					<input type="text" name="address"  placeholder="Address" value="{$user['address1']}"/>
				...
				<select id="state" name="state" required>
				  ...
				  <option value="WY">Wyoming</option>
					...
					<div class="third last">
						<input type="text" name="zip" placeholder="Zip Code" value="{$user['zipcode']}"/>
					</div>
...
$head .= <<<EOD
	...
	//Submit Error-check functions		
			$('#submit_button').click(function () {
				alert_text = "";
				error=false;
				if (($('input[name=plan]').val()).length==0){
